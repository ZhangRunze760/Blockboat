...
def mc_message_processing(message_input):
    message_input = str(message_input)
    if '@' in message_input:
        mcid = (message_input.split('@')[1]).split(' ')[0]
        qqid = QQMCBind.look_for_qqid(mcid)
        at = '@' + str(mcid) + ' '
        message_new = message_input.replace(at, '[CQ:at,qq=' + str(qqid) + ']')
        return message_new
    else:
...
while True:
    message = mc_message_processing(mc_message_get(log))
